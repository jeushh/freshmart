...
class RefundService
{
    public function create(Request $request, array $data): array
    {
        return DB::transaction(function () use ($request, $data) {
            // Resolve the product via the sale's own stored product_id first.
            // Falls back to a SKU match only for pre-migration rows that
            // predate this column (see the add_product_id_to_sales_ledger
            // migration) — new sales always have product_id set at checkout.
            ...
            // Canonical lock order: products first, then dependent ledger/refund
            // rows — matches BusinessController::checkout's lock order so a
            // concurrent refund and sale on the same product can't deadlock.
