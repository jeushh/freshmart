...
<template>
  ...
  <template v-else-if="invoice"><section class="ui-card invoice-summary"><div><h2>{{ invoice.invoice_number || 'Draft — no number' }}</h2><p>{{ invoice.supplier_name }} · {{ invoice.po_number }} · Total {{ formatMoney(total) }}</p><p v-if="invoice.notes">Notes: {{ invoice.notes }}</p></div><UiStatusBadge :status="invoice.status" /></section><p v-if="error" class="form-error" role="alert">{{ error }}</p>
    ...
    <section class="ui-card invoice-actions"><h2>Invoice actions</h2><div class="invoice-actions__buttons"><UiButton v-if="invoice.status === 'Draft'" :loading="saving" @click="transition('register')">Register invoice</UiButton><UiButton v-if="invoice.status === 'Registered'" :loading="saving" @click="transition('approve')">Approve invoice</UiButton><UiButton v-if="invoice.status === 'Registered'" variant="secondary" :disabled="saving" @click="startAction('dispute')">Dispute invoice</UiButton><UiButton v-if="invoice.status === 'Disputed'" :loading="saving" @click="transition('resolve-dispute')">Resolve dispute</UiButton><UiButton v-if="['Draft', 'Registered', 'Disputed'].includes(invoice.status)" variant="destructive" :disabled="saving" @click="startVoid">Void invoice</UiButton></div><div v-if="action === 'dispute'" class="dispute-form"><UiInput v-model="actionNotes" label="Dispute notes" maxlength="1000" hint="Optional notes recorded with the dispute." /><div class="invoice-actions__buttons"><UiButton :loading="saving" @click="transition('dispute')">Confirm dispute</UiButton><UiButton variant="secondary" :disabled="saving" @click="cancelAction">Cancel</UiButton></div></div></section>
    <UiConfirmDialog
      :open="voidOpen"
      title="Void invoice"
