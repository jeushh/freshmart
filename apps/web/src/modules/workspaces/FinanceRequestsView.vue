...
async function load(page = pagination.value.current_page) {
  ...
  } catch (requestError) { listError.value = requestError.message } finally { loading.value = false }
}
function startReview(row, decision) { review.value = { id: row.id, decision, description: row.description }; note.value = ''; reviewError.value = '' }
function cancelReview() { review.value = null; note.value = ''; reviewError.value = '' }
async function confirmReview() {
  if (!review.value) return
  submitting.value = true; reviewError.value = ''
  try {
  ...
  } catch (requestError) { reviewError.value = requestError.message } finally { submitting.value = false }
}
onMounted(() => load())
</script>
...
<template>
  ...
  <UiTableShell title="Request queue" :loading="loading" :error="listError" :empty="!requests.length" empty-title="No finance requests" empty-description="There are no finance requests to show." @retry="load">
