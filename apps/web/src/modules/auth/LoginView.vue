...
async function submit(){ error.value=''; busy.value=true; try{ await sessionStore.login(username.value,password.value); router.replace('/') }catch(e){ error.value=e.message }finally{ busy.value=false } }
</script>
